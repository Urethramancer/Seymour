...
type TimeSince struct {
	// Since when?
	Since string `short:"s" long:"since" help:"Date-time to start counting episodes from."`
}

// TimePeriod is last N <time units>.
type TimePeriod struct {
	// Period up to now.
	Period string `short:"p" long:"period" help:"Period of time to go back for episodes."`
}
